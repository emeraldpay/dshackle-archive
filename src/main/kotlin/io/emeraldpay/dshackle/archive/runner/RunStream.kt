...
class RunStream(
    ...
    fun processWindow(window: ProcessingWindow): Mono<Void> {
        val follow = client.reactorStub.subscribeHead(
            Common.Chain.newBuilder()
                .setType(Common.ChainRef.forNumber(runConfig.blockchain.id))
                .build(),
        )
            .map { it.height }
