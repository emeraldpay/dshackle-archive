...
Options:
  ...
  -c, --connection <HOST:PORT>
      ...
      --timeout <SECONDS>
          Timeout for a single API request, in seconds. Default: 15. Can also be set with `EMERALD_DSHACKLE_TIMEOUT` env; trace requests can have a separate timeout set with `EMERALD_DSHACKLE_TIMEOUT_TRACE` env
      ...
      --stream.topics-separator <STREAM_TOPICS_SEPARATOR>
          Separator placed between the `--stream.topics` prefix and the field label when building a topic name. The field labels themselves are fixed and keep their own `-` (e.g. `tx-json`), so with `.` the topics are `<prefix>.blocks`, `<prefix>.tx-json`, etc.
...
Only one of those targets can be used at a time, and a broker needs both its URL and its topic.
Anything else stops the archive at startup with an error, instead of quietly sending nothing.
...
- `pulsar://HOST:PORT` for Apache Pulsar
- `kafka://HOST:PORT` for Apache Kafka, or a comma separated list of bootstrap brokers (`kafka://kafka-1:9092,kafka-2:9092`)
...
`--stream.topics-separator` changes the `-` placed between the prefix and the field (default `-`).
E.g. with `--stream.topics-separator .` the topics become `<prefix>.blocks`, `<prefix>.tx-json`, etc., which fits the dotted naming common for Kafka.
The field labels themselves (`tx-json`, `blocks-uncles`, ...) are fixed and keep their `-`.
